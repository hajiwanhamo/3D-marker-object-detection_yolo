...
OUT = ROOT / "damage_manual_poly_D3_realstyle_v6_sat_fix_train160_val40"
...
def transform(img, mask, rng):
    ...
    # saturation 보정: v5에서 edge는 맞았지만 채도가 너무 낮았기 때문에 nonzero 영역만 채도 상승
    hsv = cv2.cvtColor(out, cv2.COLOR_BGR2HSV).astype(np.float32)
    gray_final = cv2.cvtColor(out, cv2.COLOR_BGR2GRAY)
    nz = gray_final > 0
    hsv[:, :, 1][nz] = np.clip(hsv[:, :, 1][nz] * 2.2, 0, 120)
    out = cv2.cvtColor(hsv.astype(np.uint8), cv2.COLOR_HSV2BGR)
...
def main():
    ...
    with (OUT / "v6_stats.csv").open("w", newline="", encoding="utf-8") as f:
