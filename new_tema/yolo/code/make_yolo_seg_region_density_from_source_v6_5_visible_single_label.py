...
# ============================================================
# make_yolo_seg_region_density_from_source_v6_5_visible_single_label.py
#
# 목적:
...
#   - segmentation label도 노이즈 후 보이는 point 위치를 따름
#   - 단, class별 label은 여러 조각으로 나누지 않고 1개 polygon으로 생성
#   - 목적: v5의 위치 정확도와 v6의 단일 객체 인식을 절충
#
# 입력:
#   src_dataset_root/
...
def render_full_label_mask_from_source(px_c, py_c, W: int, H: int, radius: int):
    ...
    v6.5에서는 노이즈 후 실제 보이는 point들을 사용한다.
    단, write_seg_label 단계에서 class당 1개 polygon으로 변환한다.
    """
...
def build_noisy_image_and_masks(img, px, py, cls, rng: random.Random, args):
    ...
    for cid in VALID_CLASSES:
        ...
        # 라벨은 아래에서 노이즈 후 최종 visible point 기준으로 만든다.
        # v6처럼 원본 전체 source를 그대로 쓰면 너무 넓게 잡히는 경우가 있어,
        # v6.5에서는 실제 보이는 포인트 위치를 따르되 class당 1개 객체로만 변환한다.
        ...
        # segmentation label도 실제 visible point 위치를 기준으로 만든다.
        # 이후 write_seg_label()에서 mask_to_single_full_polygon()을 사용해
        # 끊긴 조각들을 class당 1개의 minAreaRect/hull polygon으로 합친다.
        label_masks[cid] = render_point_mask(px_final, py_final, W, H, args.point_radius)
...
def mask_to_single_full_polygon(mask, W: int, H: int, args):
    """
    class 하나를 여러 조각 polygon으로 나누지 않고, 하나의 polygon으로 만든다.
    v6.5에서는 실제 visible point mask를 입력으로 받아 단일 minAreaRect/hull로 감싼다.
    """
    work = mask.copy()
...
def main():
    ...
    parser.add_argument("--label_use_full_source_single_object", action="store_true", default=True,
                        help="segmentation label을 실제 visible point 기준으로 만들고 class당 1개 polygon으로 생성")
    ...
    print("noise and seg label:     image=noisy keep mask, label=visible points single object")
