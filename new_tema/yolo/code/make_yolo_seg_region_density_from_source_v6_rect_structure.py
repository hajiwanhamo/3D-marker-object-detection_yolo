...
# ============================================================
# make_yolo_seg_region_density_from_source_v6_rect_structure.py
#
# 목적:
...
#   - 이미지는 노이즈 적용 후 남은 point로 생성
#   - segmentation label은 원본 source class별 전체 ID 영역 기준으로 생성하되, polygon을 중심 기준으로 약간 축소 가능
#   - rect1/2/3 이미지 손상은 강하게 유지하되, 긴 직사각형 방향 구조가 완전히 사라지지 않도록 생성
#   - 목적: 한 덩어리 인식은 유지하면서 rect1이 rect2/rect3으로 밀리는 class assignment 오류를 줄임
#
# 입력:
#   src_dataset_root/
...
def make_rect_keep_prob(h: int, w: int, rng: random.Random, args):
    """
    구버전 rect 손상 함수.
    현재 v6_rect_structure에서는 기본적으로 사용하지 않지만,
    point 수가 너무 적어 minAreaRect 기반 구조 손상이 불가능할 때 fallback으로 사용한다.
    """
    if h <= 1 or w <= 1:
        return np.ones((h, w), dtype=np.float32)
...
def make_rect_structure_keep_prob_for_points(px_c, py_c, rng: random.Random, args):
    """
    rect1/2/3용 구조 보존형 point keep probability 생성.

    기존 rect 손상은 bbox 내부에서 랜덤 patch를 줄이는 방식이라,
    직사각형이 짧은 조각처럼 남아 square나 다른 rect class로 밀릴 수 있었다.

    이 함수는 class별 원본 point의 minAreaRect를 기준으로 local 좌표계를 만들고,
    긴축 방향은 넓게 유지하고 짧은축 방향은 좁게 유지하는 확률 분포를 사용한다.

    결과:
    - 손상 강도는 유지
    - 듬성듬성함은 유지
    - 하지만 긴 직사각형 방향성은 완전히 사라지지 않음
    """
    if len(px_c) == 0:
        return np.zeros(0, dtype=np.float32)

    if len(px_c) < 3:
        return np.full(len(px_c), args.rect_axis_keep_min, dtype=np.float32)

    coords = np.stack([px_c, py_c], axis=1).astype(np.float32)
    rect = cv2.minAreaRect(coords.reshape(-1, 1, 2))
    (cx, cy), (w, h), angle = rect

    if w < 1.0 or h < 1.0:
        return np.full(len(px_c), args.rect_axis_keep_min, dtype=np.float32)

    # 긴 축이 local u축이 되도록 angle 보정
    if w < h:
        w, h = h, w
        angle += 90.0
    ...
    # local 좌표계
    # u: 긴축 방향, v: 짧은축 방향
    u = rel[:, 0] * cos_t + rel[:, 1] * sin_t
    v = -rel[:, 0] * sin_t + rel[:, 1] * cos_t

    half_w = max(float(w) * 0.5, 1.0)
    half_h = max(float(h) * 0.5, 1.0)
    u_norm = u / half_w
    v_norm = v / half_h

    # 전체적으로 낮은 기본 유지 확률 + 긴축 구조 보존 확률
    base_keep = rng.uniform(args.rect_axis_keep_min, args.rect_axis_keep_max)
    major_sigma = rng.uniform(args.rect_major_sigma_min, args.rect_major_sigma_max)
    minor_sigma = rng.uniform(args.rect_minor_sigma_min, args.rect_minor_sigma_max)

    structure = np.exp(
        -0.5 * ((u_norm / max(major_sigma, 1e-6)) ** 2 +
                (v_norm / max(minor_sigma, 1e-6)) ** 2)
    ).astype(np.float32)

    keep_prob = base_keep + (1.0 - base_keep) * structure

    # 지역 밀도 변화는 유지한다. 단, bbox 기준으로 local field를 곱해 실해역식 듬성듬성함을 추가한다.
    x1 = int(np.min(px_c))
    x2 = int(np.max(px_c))
    y1 = int(np.min(py_c))
    y2 = int(np.max(py_c))
    ...
    lx = np.clip(px_c - x1, 0, bw - 1)
    ly = np.clip(py_c - y1, 0, bh - 1)
    keep_prob = keep_prob * local[ly, lx]

    # 내부 국소 누락 hole 추가.
    # 긴축 구조를 완전히 끊지 않기 위해 타원형 hole만 적용한다.
    hole_count = rng.randint(args.rect_hole_count_min, args.rect_hole_count_max)
    if len(px_c) > 0:
        for _ in range(hole_count):
            idx = rng.randint(0, len(px_c) - 1)
            hu = u_norm[idx]
            hv = v_norm[idx]

            hole_major = rng.uniform(args.rect_hole_major_min, args.rect_hole_major_max)
            hole_minor = rng.uniform(args.rect_hole_minor_min, args.rect_hole_minor_max)
            reduce_ratio = rng.uniform(args.rect_hole_reduce_min, args.rect_hole_reduce_max)

            du = (u_norm - hu) / max(hole_major, 1e-6)
            dv = (v_norm - hv) / max(hole_minor, 1e-6)
            d2 = du ** 2 + dv ** 2
            hole_mask = d2 <= 1.0

            if np.any(hole_mask):
                hole_weight = (1.0 - d2[hole_mask]).astype(np.float32)
                keep_prob[hole_mask] *= (1.0 - reduce_ratio * hole_weight)

    return np.clip(keep_prob, args.keep_prob_min, args.keep_prob_max).astype(np.float32)
...
def keep_points_for_class(px_c, py_c, cls_id: int, rng: random.Random, args):
    ...
    if cls_id == 0:
        # square는 기존 중심부 보존 방식 유지
        field = make_square_keep_prob(h, w, rng, args)
        lx = np.clip(px_c - x1, 0, w - 1)
        ly = np.clip(py_c - y1, 0, h - 1)
        prob = field[ly, lx]
    else:
        # rect1/2/3은 구조 보존형 손상 사용
        prob = make_rect_structure_keep_prob_for_points(px_c, py_c, rng, args)

    np_rng = np.random.default_rng(rng.randint(0, 2**31 - 1))
    keep = np_rng.random(len(prob)).astype(np.float32) < prob

    # 너무 적게 남으면 class가 square처럼 뭉개지므로, 긴 구조 확률이 높은 point를 최소량 보존한다.
    if cls_id != 0 and len(prob) > 0:
        min_keep_count = max(int(args.rect_min_keep_points), int(round(len(prob) * float(args.rect_min_keep_ratio))))
        min_keep_count = min(min_keep_count, len(prob))
        if int(np.count_nonzero(keep)) < min_keep_count:
            top_idx = np.argsort(-prob)[:min_keep_count]
            keep[top_idx] = True
...
def render_full_label_mask_from_source(px_c, py_c, W: int, H: int, radius: int):
    """
    segmentation 정답 라벨용 mask.
    v5와 달리 노이즈 후 살아남은 조각이 아니라,
    원본 source class 전체 point를 사용한다.
    즉 입력 이미지는 듬성듬성해도 라벨은 하나의 전체 ID 객체가 되도록 한다.
    """
    return render_point_mask(px_c, py_c, W, H, radius)
...
def build_noisy_image_and_masks(img, px, py, cls, rng: random.Random, args):
    ...
    # image_masks: 노이즈 후 실제 보이는 point 기준. 이미지 시각화/통계용.
    # label_masks: segmentation 학습 label 기준. 원본 source class 전체 ID 기준.
    image_masks = {cid: np.zeros((H, W), dtype=np.uint8) for cid in VALID_CLASSES}
    label_masks = {cid: np.zeros((H, W), dtype=np.uint8) for cid in VALID_CLASSES}
    stats = {}
    ...
    for cid in VALID_CLASSES:
        ...
        # 라벨은 노이즈 적용 전 원본 source class 전체 영역으로 만든다.
        # 이 부분이 v5 대비 최소 핵심 수정이다.
        label_masks[cid] = render_full_label_mask_from_source(px[idx], py[idx], W, H, args.point_radius)
        ...
        image_masks[cid] = render_point_mask(px_final, py_final, W, H, args.point_radius)
    ...
    if args.intensity_jitter:
        ...
        for cid in VALID_CLASSES:
            final_visible_mask[image_masks[cid] > 0] = 255
    ...
    return out, label_masks, image_masks, px, py, cls, keep_all, stats
...
def shrink_polygon_points(poly_pts: np.ndarray, shrink_ratio: float, W: int, H: int):
    """
    polygon 꼭짓점을 중심 방향으로 축소한다.
    - 이미지/노이즈 생성은 건드리지 않음
    - 라벨 polygon만 약간 줄여서 v6의 과대 mask를 완화
    - shrink_ratio=1.0이면 기존 v6와 동일
    """
    pts = np.asarray(poly_pts, dtype=np.float64).reshape(-1, 2)
    ratio = float(shrink_ratio)

    if len(pts) < 3 or ratio >= 0.999:
        return pts

    # 너무 과하게 줄이면 객체 정보가 사라지므로 안전 범위로 제한
    ratio = float(np.clip(ratio, 0.50, 1.00))

    center = np.mean(pts, axis=0, keepdims=True)
    out = center + (pts - center) * ratio
    out[:, 0] = np.clip(out[:, 0], 0, W - 1)
    out[:, 1] = np.clip(out[:, 1], 0, H - 1)
    return out

def mask_to_single_full_polygon(mask, W: int, H: int, args):
    """
    class 하나를 여러 조각 polygon으로 나누지 않고, 하나의 전체 ID polygon으로 만든다.
    기본은 minAreaRect 4점 polygon이다. 회전된 직사각형 ID 전체를 하나로 감싸기 위함이다.
    """
    work = mask.copy()
    ...
    if args.label_dilate_iter > 0:
        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
        work = cv2.dilate(work, iterations=int(args.label_dilate_iter), kernel=kernel)
    ...
    if args.label_full_polygon_mode == "hull":
        hull = cv2.convexHull(pts.reshape(-1, 1, 2)).reshape(-1, 2).astype(np.float64)
        if len(hull) > args.max_polygon_points:
            sel = np.linspace(0, len(hull) - 1, args.max_polygon_points).astype(np.int32)
            hull = hull[sel]
        poly_pts = hull
    else:
    ...
    # v6_shrink 핵심: class 위치 관계는 원본 source 기준으로 유지하되, polygon만 약간 줄인다.
    poly_pts = shrink_polygon_points(poly_pts, args.label_shrink_ratio, W, H)
...
def write_seg_label(label_path: Path, class_masks, W: int, H: int, args):
    ...
    for cid in VALID_CLASSES:
        if args.label_use_full_source_single_object:
            polys = mask_to_single_full_polygon(class_masks[cid], W, H, args)
        else:
...
def process_one_image(img_path: Path, label_path: Path, source_dir: Path, out_stem: str, split: str, idx: int, variant_idx: int, args):
    ...
    noisy_img, label_masks, image_masks, px_used, py_used, cls_used, keep_all, stats = build_noisy_image_and_masks(img, px, py, cls, rng, args)

    return {
        "img": img,
        "noisy_img": noisy_img,
        "class_masks": label_masks,
...
def main():
    ...
    # rect structure-preserving noise
    parser.add_argument("--rect_axis_keep_min", type=float, default=0.18,
                        help="rect 전체에 깔리는 기본 유지 확률 최소값")
    parser.add_argument("--rect_axis_keep_max", type=float, default=0.38,
                        help="rect 전체에 깔리는 기본 유지 확률 최대값")
    parser.add_argument("--rect_major_sigma_min", type=float, default=0.85,
                        help="rect 긴축 방향 보존 sigma 최소값")
    parser.add_argument("--rect_major_sigma_max", type=float, default=1.20,
                        help="rect 긴축 방향 보존 sigma 최대값")
    parser.add_argument("--rect_minor_sigma_min", type=float, default=0.18,
                        help="rect 짧은축 방향 보존 sigma 최소값")
    parser.add_argument("--rect_minor_sigma_max", type=float, default=0.32,
                        help="rect 짧은축 방향 보존 sigma 최대값")
    parser.add_argument("--rect_hole_count_min", type=int, default=1,
                        help="rect 내부 국소 누락 hole 최소 개수")
    parser.add_argument("--rect_hole_count_max", type=int, default=4,
                        help="rect 내부 국소 누락 hole 최대 개수")
    parser.add_argument("--rect_hole_major_min", type=float, default=0.10,
                        help="rect hole 긴축 반경 최소값")
    parser.add_argument("--rect_hole_major_max", type=float, default=0.28,
                        help="rect hole 긴축 반경 최대값")
    parser.add_argument("--rect_hole_minor_min", type=float, default=0.08,
                        help="rect hole 짧은축 반경 최소값")
    parser.add_argument("--rect_hole_minor_max", type=float, default=0.22,
                        help="rect hole 짧은축 반경 최대값")
    parser.add_argument("--rect_hole_reduce_min", type=float, default=0.35,
                        help="rect hole 내부 확률 감소 최소값")
    parser.add_argument("--rect_hole_reduce_max", type=float, default=0.80,
                        help="rect hole 내부 확률 감소 최대값")
    parser.add_argument("--rect_min_keep_ratio", type=float, default=0.08,
                        help="rect가 square처럼 뭉개지지 않도록 최소 보존할 point 비율")
    parser.add_argument("--rect_min_keep_points", type=int, default=12,
                        help="rect가 square처럼 뭉개지지 않도록 최소 보존할 point 수")
    ...
    parser.add_argument("--label_use_full_source_single_object", action="store_true", default=True,
                        help="segmentation label을 노이즈 후 조각이 아니라 원본 source 전체 ID 기준 class당 1개 polygon으로 생성")
    parser.add_argument("--label_full_polygon_mode", choices=["minrect", "hull"], default="minrect",
                        help="full source label polygon 생성 방식. minrect는 회전 사각형, hull은 convex hull")
    parser.add_argument("--label_shrink_ratio", type=float, default=1.0,
                        help="원본 source 기준 polygon을 중심 기준으로 축소하는 비율. 1.0은 v6와 동일, 0.88~0.92 권장")
    ...
    print("noise and seg label:     image=rect structure noisy keep mask, label=full source ID")
    print(f"label single object:     {args.label_use_full_source_single_object}")
    print(f"label polygon mode:      {args.label_full_polygon_mode}")
    print(f"label shrink ratio:     {args.label_shrink_ratio}")
