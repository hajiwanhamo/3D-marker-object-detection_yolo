...
# - --flip_x 옵션으로 이미지/uv/meta까지 일관된 X축 좌우반전 보정 지원
...
def process_one(fp: Path, out_dir: Path, args):
    ...
    # 좌우반전 보정
    # ------------------------------------------------------------
    # 기존 가상 학습 이미지가 실해역 이미지와 좌우반전 상태였으므로,
    # --flip_x 옵션을 주면 local u 좌표 자체를 반전한다.
    #
    # 중요:
    #   단순히 이미지 pixel만 뒤집지 않고, 저장되는 uv_top / uv_all도 함께 반전한다.
    #   또한 2D -> 3D 역변환 일관성을 위해 image_u_axis_3d도 같이 반전한다.
    #
    # 결과:
    #   - *_color.png 좌우반전 보정됨
    #   - *_binary.png 좌우반전 보정됨
    #   - *_top_id_uv.npy 좌우반전 보정된 uv 저장
    #   - *_marker_all_uv.npy 좌우반전 보정된 uv 저장
    #   - *_meta.json의 image_u_axis_3d도 반전 반영
    if bool(args.flip_x):
        uv_top = uv_top.copy()
        uv_all = uv_all.copy()
        uv_top[:, 0] *= -1.0
    ...
    meta = {
        ...
        "back_projection_note": {
            ...
            "flip_x_note": "if flip_x is true, saved uv and image_u_axis_3d are already mirrored, so use the same pixel_to_uv formula",
...
def main():
    ...
    ap.add_argument(
        "--flip_x",
        action="store_true",
        help="가상데이터 projection 결과를 X축 좌우반전한다. 이미지, uv, meta의 image_u_axis_3d에 모두 반영됨",
    )
    ...
    print(f"[INFO] flip_x={args.flip_x}")
